package com.yurirvs.saiph.rag.service.handler;
...
import com.yurirvs.saiph.framework.convention.GroundingChunk;
import com.yurirvs.saiph.framework.convention.SourceRef;
import com.yurirvs.saiph.framework.web.SseEmitterSender;
import com.yurirvs.saiph.infra.chat.StreamCallback;
import com.yurirvs.saiph.infra.config.AIModelProperties;
import com.yurirvs.saiph.rag.core.memory.ConversationMemoryService;
import com.yurirvs.saiph.rag.dao.entity.ConversationDO;
import com.yurirvs.saiph.rag.dto.CompletionPayload;
import com.yurirvs.saiph.rag.dto.MessageDelta;
import com.yurirvs.saiph.rag.dto.MetaPayload;
import com.yurirvs.saiph.rag.enums.SSEEventType;
import com.yurirvs.saiph.rag.service.ConversationGroupService;
...
public class StreamChatEventHandler implements StreamCallback {
    ...
    private void initialize() {
        sender.send(SSEEventType.META.value(), new MetaPayload(conversationId, taskId));
    ...
    private void sendChunked(String type, String content) {
        ...
        while (idx < length) {
            ...
            if (count >= messageChunkSize) {
                sender.send(SSEEventType.MESSAGE.value(), new MessageDelta(type, buffer.toString()));
                buffer.setLength(0);
                count = 0;
            }
        }
