package com.yurirvs.saiph.rag.user.service.impl;
...
import com.yurirvs.saiph.rag.audit.constant.BizChangeBizType;
import com.yurirvs.saiph.rag.audit.constant.BizChangeOperationType;
import com.yurirvs.saiph.rag.audit.support.BizChangeLogContext;
import com.yurirvs.saiph.rag.user.controller.dto.ChangePasswordDTO;
import com.yurirvs.saiph.rag.user.controller.dto.UserCreateDTO;
import com.yurirvs.saiph.rag.user.controller.dto.UserPageDTO;
import com.yurirvs.saiph.rag.user.controller.dto.UserUpdateDTO;
import com.yurirvs.saiph.rag.user.controller.vo.UserVO;
import com.yurirvs.saiph.rag.user.dao.entity.UserDO;
import com.yurirvs.saiph.rag.user.dao.mapper.UserMapper;
import com.yurirvs.saiph.rag.user.enums.UserRole;
...
public class UserServiceImpl implements UserService {
    ...
    @Override
    public IPage<UserVO> pageQuery(UserPageDTO requestParam) {
        Page<UserDO> page = new Page<>(requestParam.getCurrent(), requestParam.getSize());
        LambdaQueryWrapper<UserDO> queryWrapper = Wrappers.lambdaQuery(UserDO.class)
                .eq(UserDO::getDeleted, 0)
                .and(StringUtils.hasText(requestParam.getKeyword()), wrapper -> wrapper
                        .like(UserDO::getUsername, requestParam.getKeyword())
                        .or()
                        .like(UserDO::getRole, requestParam.getKeyword()))
                .orderByDesc(UserDO::getCreateTime);
        return userMapper.selectPage(page, queryWrapper)
                .convert(each -> BeanUtil.toBean(each, UserVO.class));
    }
    ...
    @Override
    public String create(UserCreateDTO requestParam) {
    ...
    private void ensureUsernameAvailable(String username, String excludeId) {
        boolean exists = userMapper.exists(Wrappers.lambdaQuery(UserDO.class)
    ...
    private String normalizeRole(String role) {
        ...
        if (UserRole.ADMIN.getValue().equalsIgnoreCase(value)) {
            return UserRole.ADMIN.getValue();
        }
        if (UserRole.USER.getValue().equalsIgnoreCase(value)) {
            return UserRole.USER.getValue();
        }
    ...
    public void update(String id, UserUpdateDTO requestParam) {
        Assert.notNull(requestParam, () -> new ClientException("请求不能为空"));
        UserDO record = userMapper.selectById(id);
    ...
    public void changePassword(ChangePasswordDTO requestParam) {
        Assert.notNull(requestParam, () -> new ClientException("请求不能为空"));
